"""Redis 实时事件总线：为 Chat / Research 流式事件、断线续传和 Harness Durable Execution 提供共享基础设施。"""
import json
...
# 频道与锁的 key 前缀
_CHANNEL_PREFIX = "realtime:channel:"
_LOCK_PREFIX = "realtime:turn:lock:"
...
async def publish(conv_id: str, event: str, data: dict) -> None:
    """向实时频道广播一个事件。失败只记日志，不阻断主流程。"""
    try:
        payload = json.dumps({"event": event, "data": data}, ensure_ascii=False)
        await get_redis().publish(channel_for(conv_id), payload)
    except Exception as e:
        logger.warning("实时事件广播失败: conv=%s event=%s err=%s", conv_id, event, e)
...
async def acquire_turn_lock(conv_id: str) -> bool:
    ...
    try:
        ...
        logger.warning("获取实时回合锁失败: conv=%s err=%s", conv_id, e)
...
async def release_turn_lock(conv_id: str) -> None:
    try:
        ...
        logger.warning("释放实时回合锁失败: conv=%s err=%s", conv_id, e)
