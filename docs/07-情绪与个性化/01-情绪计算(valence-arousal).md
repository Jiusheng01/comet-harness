...
> 每轮对话后异步分析用户情绪，用 valence（愉悦度）-arousal（激活度）二维 + 受控情绪词表表示，滚动聚合成「当前情绪画像」，供 AI 主动关心、每日回顾和个性化交互消费。
...
- 角色：给系统加「情绪感知」——知道用户此刻什么心情，是 AI 主动关心和个性化交互的数据基础。差异化亮点。

---

## 1. 解决什么问题

- **痛点**：助手只懂内容不懂情绪。想让它感知用户心情（开心/低落/焦虑），据此调整关怀语气与个性化交互。
...
**这样情绪可量化、可比较**——系统可以观察近期情绪变化幅度、趋势与聚合结果，而不是只知道一个离散标签。同时保留一个**受控情绪词表**的主情绪标签（便于展示和理解）。

> 面试一句话：不用纯离散情绪标签而用 valence-arousal 二维连续坐标，是因为连续坐标更适合做近期聚合、趋势观察和变化幅度比较；同时保留受控词表标签便于展示。
...
| 决策点 | 选了什么 | 备选 | 为什么 |
|--------|---------|------|--------|
| 情绪表示 | valence-arousal 二维 + 词表标签 | 纯离散标签 | 坐标可聚合、可观察趋势，标签便于展示 |
...
用 valence（愉悦度 -1~1）- arousal（激活度 0~1）二维连续坐标 + 受控词表的主情绪标签。不用纯离散标签，是因为连续坐标能做近期聚合、趋势观察和变化幅度比较，离散标签主要负责可读性。
...
让计算机识别、理解、表达情感的研究领域。本项目的情绪识别 + 据情绪调整主动关心和个性化交互，属情感计算的应用。
